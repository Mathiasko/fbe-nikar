...
<objective>
Add clickable album detail pages to the photo gallery with dense masonry layout.

Purpose: Transform static gallery cards into links to immersive album pages showing all photos from each exhibition/event.
Output: Album detail pages with CSS masonry layout, gallery cards link to their albums, zero-JS maintained.
</objective>
...
**Album page approach (from user discussion):**
- Masonry/Pinterest style layout (CSS columns, no JS)
- Dense & immersive vibe (minimal gaps, photos close together)
- Easy navigation priority (clear back link, intuitive interface)
- Click any image → opens full-size in new tab (zero-JS, no lightbox)
- Album pages at /fotogaleria/[slug] and /photo-gallery/[slug]
- Each album has: title, back link, masonry grid of images
- Photos at natural aspect ratios (no forced cropping)
...
<task type="auto">
  ...
  <files>src/pages/sk/fotogaleria/[slug].astro, src/pages/en/photo-gallery/[slug].astro, src/components/GalleryCard.astro, src/components/AlbumMasonry.astro, src/styles/global.css</files>
  <action>
1. Create src/components/AlbumMasonry.astro:
   - Props: images (ImageMetadata[]), albumTitle (string)
   - Renders CSS columns masonry layout (dense, minimal gaps)
